...
- `POST /scan` - Full pipeline: detect and identify cards in image. `top_n` fixes the match count per card; omitting it switches to margin mode (`config.match_margin_pct`, default 2.0 points) -- every gallery match within that many percentage points of the best similarity is returned instead, for reprints/near-duplicates that shouldn't be arbitrarily narrowed to one. See `siglip_matcher.SigLIPCardSearch.search()` (`top_k=None`). Each matched card's response also includes `variants` (every priced finish -- see `database.py` above), since matching can't distinguish finishes that share one catalog image.
- `POST /identify` - Fast path: identify pre-cropped card (skips YOLO). Same `top_n`/margin-mode/`variants` behavior as `/scan`.
- `WS /live-recognize` - Streams JPEG camera frames (max 20 FPS); each connection gets its own tracker, so responses carry a raw per-frame `similarity` plus a stable per-connection `track_id` -- no server-side smoothing/aggregation (frontends implement their own if they want it). Takes the same `top_n`/`margin_pct` as `/scan`, but as query params on the connection URL (fixed for the connection's lifetime, alongside its tracker) rather than per-request.
